...
「노하우 시드화」 강의의 **연장통**. 참가자가 가져가서 쓸 도구만 모은 공개 페이지다.

> **연장통**이란 — 강의 참가자가 가져가서 쓸 **도구만** 모은 공개 웹 페이지.
> "연장이 아닌 것은 안 들어간다"가 이름값의 전부다.
> 정의 정본: 공유뇌 `ref/교안/연장통`.
...
강의장에서 쓴 프롬프트는 슬라이드에도 있었다. **연장통의 값어치는 집에 가서 쓸 둘이다.**
...
samples/                   막힌 사람이 중간부터 합류할 입력물
skill/seed-from-session/   스킬 소스 (SSOT)
downloads/                 스킬 배포본 + 전체 도구모음 zip (USB 배포용)
